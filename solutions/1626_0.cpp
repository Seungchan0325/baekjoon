...
ll V, E;
vector<ll> inp[MAXV];
vector<tuple<ll, ll, ll>> edges, mst, non_mst;
...
int main()
{
    ...
    ll ans = INF;
    for(auto [u, v, w] : non_mst) {
        T alt = hld_query(u, v);
        if(alt.mx1 != -1 && alt.mx1 < w) ans = min(MST + w - alt.mx1, ans);
        if(alt.mx2 != -1 && alt.mx2 < w) ans = min(MST + w - alt.mx2, ans);
    }
    cout << (ll)(ans >= INF ? -1 : ans);
}