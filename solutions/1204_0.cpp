...
ll dp(int l, int i, int j, int left)
{
    ...
    if(j == 0) {
        if(l <= L) ret = 1;
        for(int k = 0; k < N; k++) {
            if(l + s[k].size() + 1 > L) continue;
            ret += dp(l + s[k].size() + 1, k, s[k].size(), 0);
            ret %= mod;
        }
        return ret;
    }
    ...
    for(int k = 0; k < N; k++) {
        int m = s[k].size();
        if(l + s[k].size() + 1 > L) continue;
        if(checkin(s[i], s[k], j, left)) ret += dp(l + s[k].size() + 1, i, j-m, left);
        if(checkextend(s[i], s[k], j, left)) ret += dp(l + s[k].size() + 1, k, m-j, !left);
        ret %= mod;
    }
    ret %= mod;
    return ret;
