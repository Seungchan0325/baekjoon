...
int main() {
    ...
    for(int i=0; i<n; i++) {
        if(bases[i].b < bases[i].a) goto impossible;
        v[bases[i].a].push_back({bases[i].b, bases[i].idx});
    }

    for(int t=1; t<=n; t++) {
        for(auto i : v[t]) pq.push(i);
        if(pq.empty()) goto impossible;
        if(pq.top().first < t) goto impossible;
        days[pq.top().second] = t;
        pq.pop();
    }
