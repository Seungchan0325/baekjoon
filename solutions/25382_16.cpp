...
int main() {
    ...
    for(int i=0; i<n; i++) {
        int max_a = max_query(0, n, 1, 0, bases[i].y) + 1;
        max_a = max(bases[i].a, max_a);
        bases[i].a = max_a;
        max_update(0, n, 1, bases[i].y, max_a);
    }

    for(int i=n-1; i>=0; i--) {
        int min_b = min_query(0, n, 1, bases[i].y, n) - 1;
        min_b = min(bases[i].b, min_b);
        bases[i].b = min_b;
        min_update(0, n, 1, bases[i].y, min_b);
    }

    stable_sort(bases, bases + n, [](Base a, Base b) {
    ...
    for(int i=0; i<n; i++) {
        int t = i+1;
        if(bases[i].b < t || bases[i].b < bases[i].a) goto impossible;
        days[bases[i].idx] = t;
    }
