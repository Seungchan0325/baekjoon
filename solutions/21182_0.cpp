...
int main()
{
    int n;
    cin >> n;
    vector<tuple<long double, long double, long double>> ps(n);
    for(auto& [x, y, z] : ps) cin >> x >> y >> z;

    vector<Point> p;
    for(auto& [x, y, z] : ps) p.emplace_back(x, y);
    long double ans = solve(p).second;
    p.clear();
    for(auto& [x, y, z] : ps) p.emplace_back(x, z);
    ans = min(ans, solve(p).second);
    p.clear();
    for(auto& [x, y, z] : ps) p.emplace_back(y, z);
    ans = min(ans, solve(p).second);
